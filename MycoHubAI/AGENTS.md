...
- `npm run lint` runs ESLint with type-aware TypeScript, Astro, React, accessibility, and Prettier rules from @eslint.config.js.
- `npm run build` builds the Cloudflare SSR output.
- `npm run preview` previews the production build.
- `npm run format` runs Prettier with Astro and Tailwind plugins from @.prettierrc.json.
- CI in @.github/workflows/ci.yml runs `npm ci`, `npx astro sync`, lint, and build on pushes and PRs to `master`.

## Project Structure
- `src/pages/` contains Astro pages; `src/pages/api/` contains API routes with uppercase method exports.
- `src/components/` contains Astro and React components; shadcn/ui components live in `src/components/ui/`.
- `src/lib/` contains shared helpers such as Supabase and config status utilities.
- `src/middleware.ts` resolves the Supabase user and protects paths listed in `PROTECTED_ROUTES`.
- `supabase/` holds local Supabase configuration; `context/foundation/` holds product planning artifacts.
