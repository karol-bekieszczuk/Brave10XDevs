...
- [ ] 1.1 `rg -n "runTestPlan" src scripts` returns no matches.
- [ ] 1.2 `npm run test:unit` reaches actual test execution without transform errors from `runTestPlan`.
- [ ] 1.3 `npm run diagnosis:evaluate` starts without `src/lib/test-plan` module resolution errors.
- [ ] 1.4 `src/lib/diagnosis/errors.test.ts` passes the controlled redaction expectation.
- [ ] 1.5 `src/lib/diagnosis/retrieval.test.ts` expects the current `match_threshold: 20` default.

#### Manual

- [ ] 1.6 Review the API route response contract and confirm production responses no longer include `[DEBUG]` by default.
...
- [ ] 3.2 `npm run diagnosis:evaluate` passes deterministically without network/provider calls.
- [ ] 3.3 `npm run lint` passes.
- [ ] 3.4 `npm run build` passes.

#### Manual

- [ ] 3.5 Review deterministic tests and confirm they do not assert copied production prose as exact expected answers.
- [ ] 3.6 Review negative guardrail tests and confirm they test refusal behavior without endorsing unsupported features.
