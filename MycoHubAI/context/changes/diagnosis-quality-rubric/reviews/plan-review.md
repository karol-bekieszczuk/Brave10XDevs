...
- **Detail**: The local plan-review convention extracts contract surface names from H2 headings in `context/changes/diagnosis-quality-rubric/reference/contract-surfaces.md`. The plan only says to create a "concise reference registry" with entries, so an implementer could create a bullet-only file that humans can read but future tooling will not detect.
