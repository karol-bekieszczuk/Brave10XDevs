...
- [x] 1.1 PR schema boundary and normalization tests pass
- [x] 1.2 Prompt and injected-reviewer tests pass without credentials or network access
- [x] 1.3 Verdict tests cover score averages, minimum-score boundaries, and error-finding override
- [x] 1.4 Existing generic reviewer tests remain unchanged and pass
- [x] 1.5 Package type checking passes

#### Manual

- [x] 1.6 Confirm the PR-specific schema expresses only the three approved criteria and does not modify the generic `{ findings }` API
- [x] 1.7 Review representative prompts containing Markdown, shell-looking text, and prompt-injection text and confirm all PR-controlled content remains visibly delimited as data

### Phase 2: Implement Idempotent GitHub Orchestration

#### Automated

- [ ] 2.1 Event and diff tests cover normal, empty, binary, deleted, submodule, force-pushed, malformed, and oversized inputs
- [ ] 2.2 Comment and label tests prove stable-marker upsert and convergence to one result label
- [ ] 2.3 GitHub client tests cover pagination, create/update, label creation, redacted errors, and API failures
- [ ] 2.4 Orchestrator and CLI tests cover pass, fail, malformed model output, provider failure, GitHub failure, retry consumption, irrelevant labels, idempotent rerun, and stale-head rejection
- [ ] 2.5 Full package tests and type checking pass

#### Manual

- [ ] 2.6 Inspect rendered fixtures for passed, failed, and error outcomes and confirm the status, reviewed SHA, scores, findings, and retry guidance are unambiguous
- [ ] 2.7 Review fake-client call traces and confirm human comments and unrelated labels are never mutated and no secret-like value appears in output

### Phase 3: Wire the Trusted Composite Action and Repository-Root Workflow

#### Automated

- [ ] 3.1 Workflow contract tests confirm the required triggers, filters, permissions, concurrency, paths, and trusted checkout layout
- [ ] 3.2 Contract tests prove `twoj-zespol/ai-reviewer@<sha>` and `dist/review.js` are absent and the local action inputs/outputs are wired consistently
- [ ] 3.3 Clean package installation, full package tests, and package type checking pass
- [ ] 3.4 Repository formatting and diff hygiene pass

#### Manual

- [ ] 3.5 Inspect the workflow checkout paths and confirm only base-SHA automation receives the provider secret and write-capable token; no head-controlled script or action is executed
- [ ] 3.6 Confirm a fork PR, a Dependabot PR, and a non-`ai-cr:review` label event are visibly skipped by job conditions without attempting a provider call
...
- [ ] 4.1 Root static quality gates pass
- [ ] 4.2 Clean package installation and package-local gates pass
- [ ] 4.3 Documentation and YAML contain no secret values, deprecated placeholder action, nested duplicate workflow, or deployment command
- [ ] 4.4 Final repository diff passes `git diff --check` and contains only the approved change scope

#### Manual

- [ ] 4.5 A same-repository test PR to `master` automatically produces one marked comment for the current head SHA and exactly one correct `ai-cr:passed` or `ai-cr:failed` label
- [ ] 4.6 Adding `ai-cr:review` removes the command label, reruns review, updates the same comment instead of duplicating it, and leaves one result label
- [ ] 4.7 A negative review remains advisory and the workflow succeeds; a controlled operational failure produces `ai-cr:error` and a failed workflow without exposing credentials
- [ ] 4.8 Rapid updates or retry cannot leave an older head's result as the current comment/label state, and the live logs contain no provider or GitHub token
- [ ] 4.9 Confirm Cloudflare deployment configuration and behavior were not changed by this workflow
