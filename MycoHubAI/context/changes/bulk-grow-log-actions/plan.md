...
- [ ] 1.1 Bulk selection parser returns unique valid selected IDs.
- [ ] 1.2 Bulk selection parser fails when no valid selected IDs remain.
- [ ] 1.3 Repository bulk delete filters by authenticated owner ID.
- [ ] 1.4 Repository bulk delete filters by selected IDs.
...
- [ ] 1.8 The new contract does not introduce schema fields, soft delete, undo state, export, sharing, diagnosis, image, or species behavior.
- [ ] 1.9 The bulk-delete helper is a sibling of the existing single-delete helper and does not change existing single-delete behavior.

### Phase 2: Bulk Delete Route

#### Automated

- [ ] 2.1 Bulk delete API route exports uppercase `POST`.
- [ ] 2.2 Bulk delete API route rejects empty effective selection before repository deletion.
- [ ] 2.3 Bulk delete API route uses `context.locals.user.id` for owner-scoped deletion.
- [ ] 2.4 Bulk delete API route does not expose selected IDs, titles, or body text in redirect URLs.
...
- [ ] 2.8 Direct empty POST to the bulk-delete route returns to `/grow-logs` with a generic selection error.
- [ ] 2.9 Tampered malformed ID values do not delete rows and do not reveal row existence.
- [ ] 2.10 The existing single-log delete route still works.

### Phase 3: List Selection UI

#### Automated

- [ ] 3.1 `/grow-logs` builds with the bulk-delete form only when logs exist.
- [ ] 3.2 Each rendered grow-log card includes one checkbox with the selected ID as its value.
- [ ] 3.3 The bulk action posts to `/api/grow-logs/bulk-delete`.
- [ ] 3.4 Existing view/edit/detail links remain present.
- [ ] 3.5 UI copy does not imply undo or recovery.
- [ ] 3.6 Unit tests pass: `npm run test:unit`.
- [ ] 3.7 Linting passes: `npm run lint`.
- [ ] 3.8 Build passes: `npm run build`.

#### Manual

- [ ] 3.9 Owner can select multiple visible logs and trigger one delete action.
- [ ] 3.10 Browser confirmation shows the selected count before deletion.
- [ ] 3.11 Canceling the confirmation does not delete rows.
- [ ] 3.12 Submitting with no selected logs shows a generic error and deletes nothing.
- [ ] 3.13 After confirmed deletion, selected logs disappear from `/grow-logs` and unselected logs remain.
- [ ] 3.14 The page still renders the empty state when all logs are deleted.
...
- [ ] 4.4 Search confirms no `deleted_at`, undo storage, recovery queue, export, sharing, image/photo upload, species field, saved chat history, or diagnosis runtime change was added for this slice.
- [ ] 4.5 Search confirms no `/grow-logs` or bulk-delete path was added to middleware public routes.

#### Manual

- [ ] 4.6 Full local smoke test covers selecting two or more logs, canceling confirmation, confirming deletion, empty selection, malformed/tampered selection, and preserving unselected logs.
- [ ] 4.7 Local Supabase data inspection confirms selected rows are physically deleted.
- [ ] 4.8 Missing/non-owned selected IDs do not expose private row contents or ownership details.
- [ ] 4.9 Production smoke testing is explicitly left for after deployment; do not claim production verification before a deploy exists.
