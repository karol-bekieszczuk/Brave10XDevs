...
- [x] 5.1 `npm run test:unit` passes. — bd8a30b
- [x] 5.2 `npm run lint` passes. — bd8a30b
- [x] 5.3 `npm run build` passes. — bd8a30b
- [x] 5.4 `npm run diagnosis:evaluate` passes. — bd8a30b
- [x] 5.5 `rg -n "runTestPlan" src scripts` returns no matches. — bd8a30b

#### Manual

- [x] 5.6 `npm run diagnosis:evaluate:live` passes with a real `OPENROUTER_API_KEY`, Supabase service-role credentials, live-evaluation owner ID, and ingested diagnosis knowledge. — bd8a30b
- [x] 5.7 Human confirms production/public API errors remain controlled while local development detail is available only behind the dev-only boundary. — bd8a30b
- [x] 5.8 Human confirms the Phase 1 rollout status in `context/foundation/test-plan.md` matches the actual Progress state. — bd8a30b
