...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: diagnosis accuracy measurement
      decision: prepared agar/grain test cases
    - topic: grow log scope
      decision: text grow logs with full create, view, edit, and delete support plus a minimal agar/grain stage field
    - topic: AI answer shape
      decision: possible causes, suggested actions, and confidence bands with explanatory uncertainty
    - topic: chat history scope
      decision: no saved chat history; only grow logs are persisted
    - topic: outside-scope handling
      decision: refuse with redirect back to agar/grain scope
    - topic: product type
...
At 100x the initial scale, the diagnosis rule may need personalization because users will have different logging styles and cultivation setups.
...
- Non-Goals: present; scope avoids are explicitly listed.
